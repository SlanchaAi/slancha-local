...
_STATUS_JSON = {
    "Self": {
        ...
        "TailscaleIPs": ["100.91.57.17", "fd7a:115c:a1e0::1234"],
        "Online": True,
        "Tags": ["tag:specialist"],
    },
...
def test_resolve_magicdns_name_parses_live_status(monkeypatch):
    ...
    assert resolve_magicdns_name() == "promaxgb10-d325.taila93596.ts.net"
...
def test_build_node_url_uses_advertise_host_when_present():
    url = build_node_url(
        advertise_host="gb10.taila93596.ts.net", bind_host="0.0.0.0", bind_port=8000
    )
    assert url == "http://gb10.taila93596.ts.net:8000"
