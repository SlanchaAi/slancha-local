...
def _coder():
    return LocalModelDescriptor(
        ...
        capabilities=["en", "coding"], est_throughput_tps=20.0,
    )


def _general():
    return LocalModelDescriptor(
        backend="ollama", id="qwen3:8b", ctx_window=32768,
        capabilities=["en"], est_throughput_tps=40.0,
    )
...
def _reasoner():
    return LocalModelDescriptor(
        ...
        capabilities=["en", "hard"], est_throughput_tps=15.0,
    )


def _tool_caller():
    return LocalModelDescriptor(
        backend="ollama", id="qwen3-tool:8b", ctx_window=32768,
        capabilities=["en", "tool_use"], est_throughput_tps=30.0,
    )


def _select(*, domain, difficulty="medium", jailbreak=False, pii=False,
            needs_tools=False, available, escalation=True, ctx=100):
    prefs = Preferences(escalation_allowed=escalation)
    return LocalClassifier._select_target(
        domain=domain, difficulty=difficulty, jailbreak=jailbreak, pii=pii,
        needs_tools=needs_tools, available=available, preferences=prefs,
        context_len=ctx,
    )
...
class TestPerPromptDiversity:
    def test_biology_picks_generalist_not_coder_when_coder_first(self):
        target, _, reason, _ = _select(
            domain="biology", available=[_coder(), _general()],
        )
...
class TestDomainSpecific:
    def test_cs_domain_prefers_coder(self):
        target, _, _, _ = _select(
            domain="computer science", available=[_general(), _coder()],
        )
        assert target == "local:ollama:codestral:22b"

    def test_math_prefers_reasoner_over_generalist(self):
        target, _, reason, _ = _select(
            domain="math", available=[_general(), _reasoner()],
        )
    ...
    def test_hard_difficulty_picks_reasoner(self):
        target, _, reason, _ = _select(
            domain="biology", difficulty="hard",
...
class TestToolCalling:
    ...
    def test_cs_needs_tools_overrides_to_coder_not_tools(self):
        target, _, _, _ = _select(
            domain="computer science", needs_tools=True,
...
class TestEscalation:
    ...
    def test_context_overflow_with_escalation(self):
        target, _, _, _ = _select(
            domain="other", available=[_general()], ctx=1_000_000,
        )
        assert target.startswith("cloud:")
...
def test_classify_signature_passes_needs_tools(monkeypatch):
    ...
    cls._labels = {"domain": {"labels": ["other"]}, "difficulty": {"labels": ["medium"]}, "language": {"labels": ["en"]}}

    class _StubModel:
        pass

    cls._heads = {n: _StubModel() for n in ("domain", "difficulty", "language", "jailbreak", "pii", "tool_calling")}
    ...
    req = ClassifyRequest(
        ...
        prompt="x", available_models=[_general()],
        preferences=Preferences(), context_len=10,
    )
